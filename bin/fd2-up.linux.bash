...
if [ "$PROFILE" == "linux" ] || [ "$PROFILE" == "wsl" ]; then
  # If we haven't already processed both groups
  if [ -z "$GROUPS_PROCESSED" ]; then
    ...
    if [ -z "$USER_IN_FD2GRP" ]; then
      ...
      error_check
      echo "  User $(id -un) added to the fd2grp group."
    else
      echo "  User $(id -un) is in fd2grp group."
    fi

    # Create directories and save group IDs before switching groups
    rm -rf ~/.fd2/gids &> /dev/null
    mkdir -p ~/.fd2/gids
    echo "$FD2GRP_GID" > ~/.fd2/gids/fd2grp.gid
    echo "$DOCKER_GRP_GID" > ~/.fd2/gids/docker.gid

    # Export flag to avoid reprocessing and use sg to switch groups
    export GROUPS_PROCESSED=1
    exec sg docker "sg fd2grp \"$0\" \"$@\""
  fi

  # These operations are performed after groups are set up
